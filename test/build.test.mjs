...
test('buildSite emits every chapter, the table of contents, and local assets', async () => {
  ...
  try {
    const expectedChapterCount = (await loadChapters(path.resolve('translations'))).length;
    const result = await buildSite({
      rootDirectory: path.resolve('.'),
      outputDirectory,
    });

    assert.equal(result.chapterCount, expectedChapterCount);
    assert.equal(result.htmlPageCount, expectedChapterCount + 1);
    assert.equal((await findIndexFiles(path.join(outputDirectory, 'manual'))).length, expectedChapterCount + 1);
    ...
    assert.equal(searchIndex.length, expectedChapterCount);
